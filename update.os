...
Перем ПараметрыJson, ДатаОкончанияМягкойБлокировки;
Перем МассивКоманд, МассивОшибок, МассивЛога;
Перем Результат, БазаЗаблокированна;
...
Процедура ИнициализацияПараметровИПеременных()
	...
	ДатаОкончанияМягкойБлокировки = НачалоДня(ТекущаяУниверсальнаяДата());
	Результат = 0;
...
Функция локВыполнитьКомандруДеплойка(МассивКоманд, МассивОшибок)
	
	КодВозврата = 0; //нормальное заверешение работы скрипта
	мСтрокаРезультат = "";
	...
	Иначе
		...
		мНачало = МестноеВремя(ТекущаяУниверсальнаяДата(),"GMT+3");
	...
	Если КодВозврата<>0 Тогда
		ОтправитьПочту_Ошибка(мСтрокаРезультат);
	КонецЕсли;
...
Процедура ЗаблокироватьРаботуПользователей()
	...
	ДатаОкончанияМягкойБлокировки = ТекущаяУниверсальнаяДата() + ПараметрыJson["-lockstartat"];
	мРезультат = СтрШаблон("Окончание мягкой блокировки: %1",ДатаОкончанияМягкойБлокировки);
	ДобавитьЛог(ТекущаяУниверсальнаяДата(), мРезультат, 0);
	...
	мКэшРезультат = Результат;
	
	Если локВыполнитьКомандруДеплойка(МассивКоманд, МассивОшибок)<>0 Тогда
		БазаЗаблокированна = Истина;	
	Иначе
		БазаЗаблокированна = Ложь;	
	КонецЕсли;
...
Процедура ОтправитьПочту_Ошибка(ТекстСообщения)
	
	Кому = ПараметрыJson["mail"];
	
	стрБазаЗаблокированна = "";
	
	Если БазаЗаблокированна = Истина Тогда
		стрБазаЗаблокированна = "БАЗА-ЗАБЛОКИРОВАННА";	
	КонецЕсли;
	
	Тема = СтрШаблон("(ОШИБКА) Обновление %1 %2",стрБазаЗаблокированна, ПараметрыJson["-ibconnection"]);
	
	//ТекстПисьма = "ок";
	ТекстПисьма = СтрШаблон("НЕОБХОДИМО ЗАВЕРШИТЬ ОБНОВЛЕНИЕ %1 %2",Символы.ПС,ТекстСообщения);
...
//Приостановить(ПараметрыJson["-lockstartat"] * 1000); // Пауза (Миллисекунда)

ИнициализацияПараметровИПеременных();

ОтключитьСоединенияИБ(Истина); // отключения сеанса конфигуратор
ЗаблокироватьРаботуПользователей(); // Установка мягкой блокировки
ЗагрузитьКонфигурациюИзХранилища();

//ожидание времени наступления блокировки
Пока ТекущаяУниверсальнаяДата()<ДатаОкончанияМягкойБлокировки Цикл
	мРезультат = СтрШаблон("%1 - Ожидание окончания мягкой блокировки: %2",ТекущаяУниверсальнаяДата(),ДатаОкончанияМягкойБлокировки);
	Сообщить(мРезультат);
...
ОтключитьСоединенияИБ(); // отключение всех соединений 
ОбновитьКонфигурациюИБ(); // F7


Если Результат = 0 Тогда
	
	// РАЗРЕШЕНИЕ РАБОТЫ РЕГ.ЗАДАНИЯМ И ПОЛЬЗОВАТЕЛЯМ
	Если ПараметрыJson["-scheduledjobs"] Тогда	
